...
Until now, we can see the one strong point that is different between electron and e-like pion - the shower energy deposit in the ECAL. Because some of them put energy in the different section of ECAL, specially for the Late EM-showering. In this case, we thought about the variables with "longitudinal segmentation" with ECAL. We developed the discrimination variable as following:
\begin{equation}
...
In this month test beam, because we had the information of HCAL, we used the cut no more than 0.4\% energy fraction in the HCAL energy. And also, we changed some parameters of the cuts to get the best results, the cuts and parameters are as following:
\begin{table}[h]%??table???????????????????[h]???????here?????????????
    \centering
    \begin{tabular}{|c|c|c|}
    \hline
...
\begin{table}[h]%??table???????????????????[h]???????here?????????????
    ...
    $>$ 0.22 &  98.91\%+/-0.01\%(syst)+/-0.20\%(stat) &  1.56+/-0.24(stat)\\\hline  
        \end{tabular}
    \caption{E10/Etotal Electrons efficiency Tight cuts}\label{basic_3}  %????????????
\end{table}
